...
public class GestureRecognizer : MonoBehaviour
{
	...
	static int angleRange = 55;					// Angle detail level of when matching with templates 
	...
	static void gestureMatch(ArrayList pointArray)
	{
		...
		if (score < 0.7f)
		{
			...
			// Gesture.GetComponent<GUIText>().text = "RESULT: NO MATCH " + "\n" + "SCORE: " + Mathf.Round(100 * score) + "%";
		}
		else
		{
			if((DrawGM.quest==count) || (DrawGM.quest == count+8))
			{
				DrawGM.Lvl++;
				DrawGM.TimeLeft = 10 - (DrawGM.Lvl*0.33f);
				DrawGM.Score++;
				DrawGM.NewQuest();
				DrawGM.result = 1;
			}

			Debug.Log("RESULT: " + GestureTemplates.TemplateNames[count] + " SCORE: " + score + "count: " + count);
			// Gesture.GetComponent<GUIText>().text = "RESULT: " + GestureTemplates.TemplateNames[count] + "\n" + "SCORE: " + Mathf.Round(100 * score) + "%";
		}
